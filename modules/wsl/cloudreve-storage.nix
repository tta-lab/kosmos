    ...
    script = ''
      while true; do
        ...
        if [ "$mounted_uuid" = "${diskUuid}" ]; then
          ...
          marker="$(mktemp ${lib.escapeShellArg "${mountPoint}/.kosmos-storage-ready.XXXXXX"})"
          printf '%s\n' ${lib.escapeShellArg diskUuid} >"$marker"
          chmod 0644 "$marker"
          mv -f "$marker" ${lib.escapeShellArg "${mountPoint}/.kosmos-storage-ready"}
