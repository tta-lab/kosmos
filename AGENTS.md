...
`~/.config/kepos/peer.toml` is an intentional unmanaged runtime output,
not a Home Manager file. Do not edit it directly: keep peer keys, ACLs,
and service entries in `kepos/peer-policy.jsonnet`, then render
...
- **Never edit managed `~/.config/*` files directly** — edit the repo source and rebuild WSL. The project registry in `~/.config/ttal/` is unmanaged; use `og clone` for normal project additions and edit the registry directly only for archive/migration work. Kepos's unmanaged `~/.config/kepos/peer.toml` is generated output: edit `kepos/peer-policy.jsonnet` and run `just kepos-policy-render` instead.
