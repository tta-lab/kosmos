...
After creating those directories on the verified filesystem, the service writes
`.kosmos-storage-ready` at the mount root. Cloudreve, Ente, and Navidrome
mount the marker read-only and compare its value to the expected UUID in an
init-container startup gate; it is absent whenever the Micron filesystem is
not mounted.
