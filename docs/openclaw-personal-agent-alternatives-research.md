...
| Image input | Yes when the selected model declares image input. | Yes in the TUI/RPC for vision-capable models (clipboard, drag/drop, or API content blocks), but that is terminal ingress rather than a phone chat attachment flow. [Pi session format](https://github.com/earendil-works/pi-mono/blob/main/packages/coding-agent/docs/session-format.md) |
...
| Phone and TTS | Kosmos no longer deploys DSH. Upstream treats narrow-phone UI as a work item and usable mobile layouts as community plugins; no stable built-in TTS surface was established. [mobile issue](https://github.com/deepseek-ai/deepseek-harness/discussions/1721) | Android support is a Termux guide, not a mobile chat client; no documented core TTS. A good phone client, voice pipeline and push delivery would be separate products. [Pi README](https://github.com/earendil-works/pi-mono/blob/main/packages/coding-agent/README.md) |
| Stability / recommended role | Upstream still labels DSH a developer preview. Kosmos does not deploy it. | Pi's compact, well-documented extension API makes it a good **coding agent or embedded worker**. Turning it into the companion gateway would add a mobile channel, persistent service, scheduler, TTS and memory outbox—well beyond the requested small gap-filling layer. |
