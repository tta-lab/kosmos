...
Use FlickNote MCP `note_add` with `project: "orientation"` to save one note:
...
Use `note_get` to re-read the saved note as the implementer. Fix missing paths, hidden assumptions, unclear ordering, and unresolved decisions with `note_modify`, `note_replace_section`, or `note_append`.
