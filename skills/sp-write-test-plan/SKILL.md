...
- Search prior designs, research, and bug reports with FlickNote MCP `note_find`.
...
Use `note_add` with `project: "orientation"` to save the plan:
...
Use `note_get` to re-read the note as the test implementer:
...
Update the note with `note_modify`, `note_replace_section`, or `note_append` as appropriate.
...
Return the FlickNote ID and a concise summary. Stop; do not execute the plan or invoke another skill.
